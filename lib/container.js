...
/**
 * TODO: this will eventually call makeContainer after processing the data
 ...
 */
function convertResource(data, dir, opts) {
    // validate opts
    if(!opts.language || !opts.project || !opts.resource || !opts.container_type) {
        return Promise.reject(new Error('Missing required parameters'));
    }

    const writeFile = promiseUtils.promisify(fs, 'writeFile');
    const mimeType = opts.project.slug !== 'obs' && opts.container_type === 'book' ? 'text/usfm' : 'text/markdown';
    ...
    return new Promise(function(resolve, reject) {
        ...
        .then(function() {
            ...
            return writeFile(path.join(dir, 'LICENSE.md'), new Buffer(opts.resource.status.license));
        })
        .then(function() {
            // content
            return new Promise(function(resolve, reject) {
                try {
                    ...
                    if(opts.container_type === 'book') {
                        if (opts.project.slug === 'obs') {
                            // add obs images
                    ...
                    } else if(opts.container_type === 'help') {
                        if(opts.resource.slug === 'tn') {
                            data = JSON.parse(data);
                    ...
                    } else if(opts.container_type === 'dict') {
                        data = JSON.parse(data);
                        let config = {};
                        ...
                        reject(new Error('Unsupported resource container type ' + opts.container_type));
                    }
                } catch(err) {
                    console.error(data);
                    reject(err);
        ...
        .then(function() {
            if(!opts.close) {
                ...
                return openContainer(dir);
            } else {
                // finish with closed container
                return closeContainer(dir);
            }
        })
        .catch(function(err) {
