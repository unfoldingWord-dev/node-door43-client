...
function Library(sqliteHelper, opts) {
    ...
    this.addVersification = function(versification,  source_language_id) {
        ...
        var versification_id = firstId(query('select id from versification where slug=? order by id asc', [versification.slug]));
    ...
    this.getParentCategory = function(languageSlug, childCategorySlug) {
        var result = query('select pcn.name, pc.slug from category as pc' +
    ...
    this.getProjectMeta = function(projectSlug) {
        var result = query('select * from project where slug=? limit 1', [projectSlug]);
        if(result.length > 0) {
            delete result[0].id;
    ...
    this.public_getters = {
         ...
         */
        getMetrics: function() {
            var resultLangCount =  query('select count(*) as count from target_language');
            var resultResLvl3Count = query('select count(*) as count from resource where checking_level >= 3');
            var resultResCount = query('select count(*) as count from resource');
            var resultProjCount = query('select count(*) as count from (select id from project group by slug)');
            return {
        ...
        getCategory(languageSlug, slug) {
            var result = query('select cn.name, c.slug from category as c' +
        ...
        findTranslations: function(language_slug, project_slug, resource_slug, resource_type, translate_mode, min_checking_level, max_checking_level) {
            ...
            var translations = [];
            var results = query("select l.slug as language_slug, l.name as language_name, l.direction," +
            ...
            for(var r of results) {
                ...
                var imported = false;
                for(var f of formatResults) {
                    delete f.id;
                    delete f.resource_id;
                    f.imported = f.imported ? true : false;
                    formats.push(f);
                    // TRICKY: this is not technically correct, but for convenience we attach the import status to the resource directly
