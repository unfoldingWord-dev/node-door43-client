...
function Library(sqliteHelper, opts) {
    ...
    this.addResource = function(resource, project_id) {
        validateNotEmpty(resource, ['slug', 'name', 'type']);
    ...
    this.public_getters = {
        ...
        getProjectCategories: function(parentCategoryId, languageSlug, translateMode) {
            ...
            var projects = query('select \'project\' as type, \'\' as source_language_slug,' +
                ' p.id, p.slug, p.sort, p.name from project as p' +
                ' where p.category_id=? group by p.slug', [parentCategoryId]);
            // find best name
