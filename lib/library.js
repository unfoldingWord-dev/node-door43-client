...
function Library(sqliteHelper, opts) {
    ...
    this.public_getters = {
         ...
         * @param translateMode {string} limit the results to just those with the given translate mode
         * @returns {[{}]} an array of project categories
         */
        getProjectCategories: function(parentCategoryId, languageSlug, translateMode) {
            // TODO: use the translateMode to filter further
            var categories = query('select \'category\' as type, category.slug as name, \'\' as source_language_slug, * from category where parent_id=?', [parentCategoryId]);
            ...
            for(var cat of categories) {
                for(var slug of preferredSlug) {
                    var result = query(catNameQuery, [slug]);
                    if(result.length > 0) {
                        cat.name = result['name'];
                        cat.source_language_slug = result['source_language_slug'];
                        break;
                    }
                }
            }
            // var categories = query('select \'category\' as type,' +
            //     ' coalesce(sl.slug, sl_en.slug, sl_default.slug) as source_language_slug,' +
            //     ' c.id, c.slug, c.parent_id, cn.name from category as c' +
            //     ' left join category_name as cn on cn.category_id=c.id' +
            //     ' left join source_language as sl on sl.id=cn.source_language_id and sl.slug=:language_slug' +
            //     ' left join source_language as sl_en on sl_en.id=cn.source_language_id and sl_en.slug=\'en\'' +
            //     ' left join (select * from source_language) as sl_default on sl_default.id=cn.source_language_id ' +
            ...
            // var projects = query('select \'project\' as type');

            var projects = query('select \'project\' as type,' +
                ' coalesce(sl.slug, sl_en.slug, sl_default.slug) as source_language_slug,' +
                ' p.id, p.slug, p.sort, p.name from project as p' +
                ' left join source_language as sl on sl.id=p.source_language_id and sl.slug=:language_slug' +
                ' left join source_language as sl_en on sl_en.id=p.source_language_id and sl_en.slug=\'en\'' +
                ' left join (select * from source_language) as sl_default on sl_default.id=p.source_language_id ' +
        ...
        getResources: function(languageSlug, projectSlug) {
            ...
            if(languageSlug) {
                // filter by language
                result = query('select * from resource' +
                    ' where project_id in (' +
                    '  select id from project where slug=? and source_language_id in (' +
                    '   select id from source_language where slug=?))' +
                ...
                result = query('select sl.slug as source_language_slug, r.* from resource as r' +
                    ' left join project as p on p.id=r.project_id' +
                    ' left join (' +
