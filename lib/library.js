...
function library(sqliteHelper) {
    ...
    this.addSourceLanguage = function(language) {
        ...
        save();
        return firstId(zipper(query('select id from source_language where slug=\'?\' order by id desc', [language.slug])));
    };
    ...
    this.addTargetLanguage = function(language) {
        ...
        save();
        return firstId(zipper(query('select id from target_language where slug=\'?\' order by id desc',
            [language.slug])));
    };
    ...
    this.addProject = function(project, source_language_id) {
        ...
        return firstId(zipper(query('select id from project where slug=\'?\' and source_language_id=? order by id desc',
    ...
    this.addVersification = function(versification,  source_language_id) {
        ...
        var versification_id = firstId(zipper(query('select id from versification where slug=\'?\' order by id desc', [versification.slug])));
        run('insert or ignore into versification_name' +
            ' (source_language_id, versification_id, name) values (:lang, :vers, \':name\');' +
            'update versification_name set name=\':name\'' +
            ' where source_language_id=:lang and versification_id=:vers', {
            lang: source_language_id,
            vers: versification_id,
     ...
     * @param chunk
     * @param project_id the project that this marker exists in
     * @param versification_id the versification this chunk is a member of
     */
    this.addChunkMarker = function(chunk, project_id, versification_id) {
        run('insert or ignore into chunk_marker' +
            ' (chapter, verse, versification_id, project_id)' +
        ...
        save();
        return firstId(zipper(query('select id from chunk_marker' +
            ' where versification_id=?' +
            ' and project_id=?' +
            ' order by id desc', [versification_id, project_id])));
    };
    ...
    this.addCatalog = function(catalog) {
        ...
        save();
        return firstId(zipper(query('select id from catalog where slug=\'?\' order by id desc', [catalog.slug])));
    };
    ...
    this.addResource = function(resource, project_id) {
        ...
        var resourceId = firstId(zipper(query('select id from resource where slug=\'?\' and project_id=? order by id desc',
    ...
    this.getters = {
        ...
        getChunkMarkers: function(projectSlug, versificationSlug) {
            return zipper(query('' +
                ...
                ' where v.slug=\'?\' and p.slug=\'?\'', [versificationSlug, projectSlug]));
        }
    };
