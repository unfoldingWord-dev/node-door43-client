...
function Library(sqliteHelper, opts) {
    ...
    this.addTargetLanguage = function(language) {
        validateNotEmpty(language, ['slug', 'name', 'direction']);
        let id = insertUpdate('target_language', {
            slug: language.slug,
            name: language.name,
    ...
    this.addTempTargetLanguage = function(language) {
        validateNotEmpty(language, ['slug', 'name', 'direction']);
        let id = insertUpdate('temp_target_language', {
            slug: language.slug,
            name: language.name,
            anglicized_name: language.anglicized_name,
            direction: language.direction,
    ...
    this.addQuestionnaire = function(questionnaire) {
        ...
        return insertUpdate('questionnaire', {
    ...
    this.public_getters = {
        ...
        getProjectCategories: function(parentCategoryId, languageSlug, translateMode) {
            ...
            var categories = query('select \'category\' as type,' +
                ...
                ' left join source_language as sl_en on sl_en.id=cn.source_language_id and sl.slug=\'en\'' +
                ' left join (select * from source_language LIMIT 1) as sl_default on sl_default.id=cn.source_language_id ' +
            ...
            var projects = query('select \'project\' as type,' +
                ...
                ' left join source_language as sl_en on sl_en.id=p.source_language_id and sl.slug=\'en\'' +
                ' left join (select * from source_language LIMIT 1) as sl_default on sl_default.id=p.source_language_id ' +
