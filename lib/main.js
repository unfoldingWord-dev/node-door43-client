...
function API(dbPath, resourceDir, opts) {
    ...
    const downloadFutureCompatibleResourceContainer = function(languageSlug, projectSlug, resourceSlug) {
        ...
        return new Promise(function(resolve, reject) {
                ...
                // try {
                //     fs.statSync(path.join(dir + '.tstudio'));
            ...
            .then(function(resource) {
                ...
                if(resource && resource.formats) {
                    ...
                    var containerType = containerFormat.mime_type.split('+')[1];
                    destFile = path.join(resourceDir, languageSlug + '_' + projectSlug + '_' + containerType + '_' + resourceSlug + '.tstudio');
        ...
        return new Promise(function(resolve, reject) {
            ...
            .then(function() {
                if(opts.compression_method === 'zip') {
                    return utils.zip(dir, dir + '.tstudio');
                } else {
                    return utils.tar(dir, dir + '.tstudio');
                }
            })
            .then(function() {
                rimraf.sync(dir);
