...
function Client(dbPath, resourceDir, opts) {
    ...
    /**
     * Parses the changed (approved) temporary language codes catalog and indexes it.
     *
     * @param data {string}
     * @param onProgress {function} an optional progress listener. This should receive progress id, total, completed
     * @returns {Promise}
    ...
    const convertLegacyResourceToContainer = function(languageSlug, projectSlug, containerType, resourceSlug, data) {
        ...
        return new Promise(function(resolve, reject) {
                ...
                props.language = library.getters.getSourceLanguage(languageSlug);
                if(!props.language) reject(new Error('Missing language'));
                props.project = library.getters.getProject(languageSlug, projectSlug);
                if(!props.project) reject(new Error('Missing project'));
                props.resource = library.getters.getResource(languageSlug, projectSlug, resourceSlug);
                if(!props.resource) reject(new Error('Missing resource'));
    ...
    /**
     * Returns a list of resource container paths
     ...
     * If no language is given the results will include all projects in all languages not just projects in a single language. This is helpful if you need to view updates based on project first rather than source language first.
     *
