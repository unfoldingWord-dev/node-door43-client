...
function API(dbPath, resourceDir, opts) {
        ...
        return request.read(project.resourceUrl)
            .then(function(response) {
                ...
                for(var resource of data) {
                    ...
                    // TODO: convert questions, notes, words to projects
                }
            });
    };
     ...
     * with downloadFutureCompatibleResourceContainer (which should be renamed to downloadResourceContainer).
    ...
    const downloadResourceContainer = function(languageSlug, projectSlug, resourceSlug) {
        ...
        return downloadFutureCompatibleResourceContainer(languageSlug, projectSlug, resourceSlug)
            .then(function(path) {
                ...
                var containerType = containerFormat.mime_type.split('+')[1];
                var data = fs.readFileSync(path).toString();
                rimraf.sync(path);
                return makeResourceContainer(languageSlug, projectSlug, containerType, resourceSlug, data);
            });
    };
     ...
     * should be renamed to downloadResourceContainer
     *
     * @param languageSlug
     * @param projectSlug
     * @param resourceSlug
     ...
     */
    const downloadFutureCompatibleResourceContainer = function(languageSlug, projectSlug, resourceSlug) {
        var destFile;
        return new Promise(function(resolve, reject) {
            ...
            .then(function(resource) {
                ...
                if(resource && resource.formats) {
                    var containerFormat = getResourceContainerFormat(resource.formats);
                    if(containerFormat == null) throw new Error('Missing resource container format');
                    var containerType = containerFormat.mime_type.split('+')[1];
                    destFile = path.join(resourceDir, languageSlug + '_' + projectSlug + '_' + containerType + '_' + resourceSlug + '.' + containerExt);
                    ...
                    if (fileExists) throw new Error('Resource container already exists');
    ...
    const makeResourceContainer = function(languageSlug, projectSlug, containerType, resourceSlug, data) {
        ...
        return new Promise(function(resolve, reject) {
                try {
                    fs.statSync(path.join(dir + '.' + containerExt));
            ...
            .then(function() {
                if(opts.compression_method === 'zip') {
                    return utils.zip(dir, dir + '.' + containerExt);
                } else {
                    return utils.tar(dir, dir + '.' + containerExt);
                }
            })
            .then(function() {
                rimraf.sync(dir);
    ...
    return {
        ...
        downloadFutureCompatibleResourceContainer: downloadFutureCompatibleResourceContainer,
