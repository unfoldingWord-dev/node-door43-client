...
function Client(dbPath, resourceDir, opts) {
     ...
     * If the container already exists in the system it will be updated.
     ...
     * @param force {boolean} force the import to proceed even if the container already exists
     * @returns {Promise.<Container>}
     */
    const importContainer = function(containerPath, force) {
        return rc.load(containerPath)
            .then(function(newContainer) {
                // check for existing container
                var newInfo = newContainer.info();
                return openContainer(newInfo.language.slug, newInfo.project.slug, newInfo.resource.slug)
                    .then(function(oldContainer) {
                        if(!force) return Promise.reject(new Error('The resource container already exists'));
                        // force import the container
                        return copyContainer(newContainer);
                    })
                    .catch(function() {
    ...
    /**
     * Copies a resource container into system.
     *
     * @param container {Container} the resource container that will be copied.
     * @returns {Promise.<Container>}
     */
    const copyContainer = function(container) {
        return new Promise(function(resolve, reject) {
            // TODO: get destination
            var destination = '';
            ncp.limit = 10; // more than enough depth for rc's
