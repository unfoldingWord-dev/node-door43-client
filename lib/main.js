...
function Client(dbPath, resourceDir, opts) {
            ...
            return listResourceContainers()
                .then(function(results) {
                    ...
                    for(var l of languages) {
                        if(!local[l.slug] || l.modified_at > local[l.slug]) updates.push(l.slug);
                    }
    ...
    const listProjectUpdates = function() {
       return new Promise(function(resolve, reject) {
            try{
                resolve(library.listProjectsLastModified());
            } catch (err) {
                reject(err);
            }
        }).then(function(projects) {
            return listResourceContainers()
                .then(function(results) {
                    ...
                    for(var p of projects) {
                        if(!local[p.slug] || p.modified_at > local[p.slug]) updates.push(p.slug);
                    }
                    return Promise.resolve(updates);
                });
        });
