...
function Client(dbPath, resourceDir, opts) {
        ...
        return request.read(project.chunks_url)
            .then(function(response) {
                ...
                if(versification) {
                    ...
                    console.error('Unknown versification ' + versification_slug);
                }
            });
    };
     ...
     * @returns {Promise}
     */
    const updateCatalogIndex = function(slug) {
        return new Promise(function(resolve, reject) {
            ...
            .then(function(data) {
                if(slug === 'langnames') {
                    return indexTargetLanguageCatalog(data);
                } else if(slug === 'new-language-questions') {
                    return indexNewLanguageQuestionsCatalog(data);
                } else if(slug === 'temp-langnames') {
                    return indexTempLangsCatalog(data);
                } else if(slug === 'changed-temp-langnames') {
                    return indexChangedTempLangsCatalog(data);
                } else {
     ...
     * @returns {Promise}
     */
    const indexTargetLanguageCatalog = function(data) {
        return new Promise(function(resolve, reject) {
     ...
     * @returns {Promise.<*>}
     */
    const indexNewLanguageQuestionsCatalog = function(data) {
        return Promise.reject(new Error('Parsing the new language questions catalog has not been implemented yet.'));
    };

    const indexTempLangsCatalog = function(data) {
       return Promise.reject('not implemented');
    };

    const indexChangedTempLangsCatalog = function(data) {
        return Promise.reject('not implemented');
    };
                    ...
                    try {
                        ...
                        if(containerType === 'book') {
                            ...
                            for(let word of data) {
                                ...
                                mkdirp.sync(wordDir);
                                let body = '#' + word.term + '\n\n' + word.def;
                                fs.writeFileSync(path.join(wordDir, '01.' + ext), body);
