...
function API(dbPath, resourceDir) {
    ...
    const downloadSourceLanguages = function(project) {
        return request.read(project.lang_catalog)
            .then(function(response) {
                ...
                // TRICKY: we just flipped the data hierarchy from lang->project to project->lang for future compatibility
                return projects;
            });
    };
    ...
    this.downloadCatalogTest = function(url, projectSlug) {
        ...
        return request.read(url).then(function(response) {
           if(response.status !== 200) return false;
            ...
            var catalog = library.getters.getCatalog('langnames');
            promises.push(request.read(catalog.url).then(function(response) {
                var data = JSON.parse(response.data);
                var count = 0;
            ...
            // index projects
            promises.push(utils.chain(downloadSourceLanguages, function(err, data) {
    ...
    this.downloadCatalog = function(url) {
        ...
        return request.read(url).then(function(response) {
            if(response.status !== 200) return false;
            library.addCatalog('langnames', 'http://td.unfoldingword.org/exports/langnames.json', 0);
            library.addCatalog('new-language-questions', 'http://td.unfoldingword.org/api/questionnaire/', 0);
